...
- transport_layer: Webview forms with validated message payloads and approved script/core contracts.
- user_reachable_surface: Task-to-BR capture form; create project and create feature remain terminal actions until shell-runtime contracts are accepted.
- gap_status: implemented_for_approved_contract
- repo_impact: extension, shell-runtime
- evidence: `requirements_ears.md` Requirement 8
- gap_to_close: Task-to-BR guided form is implemented with Webview and extension-host validation, shared-core delegation, visible success/cancellation/failure states, and dashboard refresh after successful capture.
...
- gap_status: implemented
- repo_impact: extension, asdlc-coordinator
- evidence: `requirements_ears.md` Requirement 10
- gap_to_close: Implemented by the feature-level capture form and extension-host action that calls `overmind capture task-to-br` or the bundled core API, then refreshes feature state.
...
- current_state: Implemented with TypeScript build/test/package scripts, local VSIX output, operator guide, internal release guide, and release checklist.
- required_behavior: Provide VS Code extension activation, commands, Webview registration, and extension packaging.
- gap_to_close: Complete cross-platform manual smoke records for macOS, Linux, and Windows Remote WSL before broad internal distribution.
...
- current_state: Implemented for the read-only dashboard, artifact links, refresh, terminal actions, and the approved task-to-BR guided capture form.
...
- gap_to_close: Future create-project and create-feature forms still require accepted non-interactive shell-runtime contracts.
...
- current_state: Implemented for allow-listed terminal script actions and task-to-BR shared-core capture.
...
- gap_to_close: Add more non-interactive action handlers only after their script contracts are accepted.
...
- current_state: Existing scripts are primarily CLI/interactive; the current contract inventory is recorded in `requirements/non_interactive_script_contracts.md`.
- required_behavior: Existing interactive scripts can be launched in a visible terminal; future non-interactive form flows require stable script input contracts. Task-to-BR capture is the only currently approved non-interactive flow and must use `overmind capture task-to-br` or the bundled core API.
...
- current_state: Core capture primitive exists as `overmind capture task-to-br`; the extension now delegates task-to-BR capture requests to that primitive instead of rendering canonical `user_br_input.md` content.
...
- gap_to_close: No remaining extension-host delegation gap for the approved task-to-BR capture form.
