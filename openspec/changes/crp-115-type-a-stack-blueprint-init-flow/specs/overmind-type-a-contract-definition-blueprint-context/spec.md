...
### Requirement: Type A Step 2 waits for approved stack-family blueprints
For project type `A`, Step `2` SHALL run only after Step `1.1` has produced all required active-class stack-family blueprints and those blueprints pass the CRP-114 quality helper.

#### Scenario: Step 2 blocked before blueprints exist
- **WHEN** a type `A` project has active classes and required stack-family blueprints are missing
- **THEN** Step `2` cannot be considered ready

#### Scenario: Step 2 ready after blueprints pass quality
- **WHEN** a type `A` project has all required active-class stack-family blueprints and each passes quality validation
- **THEN** Step `2` can proceed

### Requirement: Step 2 treats blueprints as read-only project context
For project type `A`, Step `2` SHALL treat approved stack-family blueprints as read-only project context. Step `2` SHALL NOT modify stack blueprint files.

#### Scenario: Step 2 receives blueprint context
- **WHEN** Step `2` runs for a type `A` project
- **THEN** the prompt/context includes the required active-class stack-family blueprint paths as read-only inputs
...
### Requirement: Blueprints are not contract schema definitions or structural evidence
Step `2` SHALL NOT treat stack-family blueprints as API contract schemas, shared request/response definitions, repository scan evidence, or Step `7` surface-map evidence. Stable cross-project contract governance SHALL remain owned by `common_contract_definition.md`.

#### Scenario: Blueprint stack family informs context only
- **WHEN** a type `A` blueprint declares an approved stack-family choice
