...
- [ ] 1.1 Confirm `project_stack_blueprint_be_TEMPLATE.md`, `project_stack_blueprint_fe_TEMPLATE.md`, and `project_stack_blueprint_mobile_TEMPLATE.md` exist from CRP-114
- [ ] 1.2 Confirm `check_project_stack_blueprint_quality.sh` exists and can validate backend, frontend, and mobile blueprint artifacts
- [ ] 1.3 Confirm CRP-115 implementation does not add workflow-state fields to CRP-114 templates
...
- [ ] 3.2 Add `required_if` conditions so Step `1.1` requires one `project_stack_blueprint_<class>.md` per active class only for project type `A`
- [ ] 3.3 Ensure Step `1.1` is non-blocking for project types `B` and `C`
- [ ] 3.4 Add tests proving type `A` Step `1.1` blocks until every active-class blueprint exists and passes quality
- [ ] 3.5 Add tests proving type `B` and `C` init can proceed without stack blueprints

## 4. Blueprint Authoring Command And Rule
...
- [ ] 4.6 Make the command run `check_project_stack_blueprint_quality.sh` after writing each final blueprint
- [ ] 4.7 Make the command support revising an existing blueprint through the same approval and quality-validation path when stable stack conventions change

## 5. Guidance And Fallback Authoring Flow

- [ ] 5.1 Add prompt/rule behavior that uses configured per-class stack guidance when available
...
- [ ] 6.1 Update `overmind/scripts/init_common_contract_definition.sh` so type `A` requires completed Step `1.1` blueprints before model invocation
- [ ] 6.2 Bind active-class blueprint paths into Step `2` prompt/context as read-only inputs for type `A`
- [ ] 6.3 Ensure Step `2` snapshots and verifies blueprints remain unchanged after model invocation
- [ ] 6.4 Update Step `2` rule/prompt text so blueprints are project context only, not API contract schema definitions
...
- [ ] 7.7 Update `tests/ai_scripts/project_setup_asdlc_tests.sh` for optional per-class guidance metadata
- [ ] 7.8 Update `tests/ai_scripts/init_common_contract_definition_tests.sh` for type `A` blueprint read-only context
