...
### Requirement: Step 8.1.5 position and ordering
The feature pipeline SHALL include a required Step `8.1.5` positioned after Step `8.1` (implementation slices) and before Step `8.2` (implementation plan); Step `8.2` SHALL NOT start until Step `8.1.5` is complete.

#### Scenario: Step 8.1.5 runs after slices, before plan
- **WHEN** the e2e orchestrator advances past Step `8.1`
- **THEN** the orchestrator SHALL invoke Step `8.1.5` next and SHALL NOT invoke Step `8.2` until `check_prerequisite_gaps_quality.sh` exits with status 0

#### Scenario: Skipping Step 8.1.5 is not permitted
- **WHEN** the orchestrator evaluates whether Step `8.1.5` may be skipped
- **THEN** the orchestrator SHALL treat Step `8.1.5` as a non-optional required step and SHALL refuse to advance to `8.2` without it

#### Scenario: Resume flag accepts 8.1.5 as a step identifier
- **WHEN** the orchestrator is invoked with `--resume 8.1.5`
- **THEN** the orchestrator SHALL resume from Step `8.1.5` and SHALL accept `prerequisite-gap-trace` and `prerequisite-gaps` as alias identifiers for this step
...
Before Step `8.2` may start, every `unmet` prerequisite in `prerequisite_gaps.md` SHALL be promoted to `implementation_slices.md` as a scheduled slice and the corresponding entry SHALL be updated to `status: scheduled_in_slices` with a valid `slice_ref`.
...
- **THEN** `check_prerequisite_gaps_quality.sh` SHALL be re-run and SHALL pass before the orchestrator advances to Step `8.2`
