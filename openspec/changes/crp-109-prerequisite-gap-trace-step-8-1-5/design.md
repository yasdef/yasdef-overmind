...
**Goals:**
- Insert a new required Step `8.2` between `8.1` (slices) and `8.3` (plan) that produces `prerequisite_gaps.md`.
...
- Gate Step `8.3` start on zero `unmet` entries in `prerequisite_gaps.md`.
...
- Add rule, template, golden example, generator script, and quality helper for Step `8.2`.
...
- Do not enforce prerequisite reasoning on Steps other than `8.2` and `8.3`.

## Decisions

**1. Step 8.2 is mandatory and positioned between 8.1 and 8.3**
Rationale: slices are the canonical scheduling source and must exist before the prerequisite trace can evaluate what is `scheduled_in_slices`. The plan (`8.3`) must not start until the prerequisite gate is clear, so `8.2` sits between them as a hard dependency. Making it optional would allow the failure mode it is designed to prevent.
Alternative: fold prerequisite tracing into `8.1` or `8.3`. Rejected — merging the gate into an existing step makes the rule harder to audit and allows a single large step to silently skip prerequisite reasoning.
...
**4. `unmet` prerequisites must be promoted to `implementation_slices.md` before `8.3`**
...
Alternative: allow `8.3` to add steps for `unmet` prerequisites inline. Rejected — `implementation_slices.md` is the canonical scheduling source; adding prerequisites only in the plan skips slice-level visibility and breaks the `crp-098` coverage check.
...
- [Risk] `prerequisite_gaps.md` may become stale if `implementation_slices.md` is edited after `8.2` runs.
  Mitigation: the quality helper for `prerequisite_gaps.md` validates that every `scheduled_in_slices` entry has a `slice_ref` that resolves in the current `implementation_slices.md`. A stale entry will cause a gate failure and force a re-run of `8.2`.
...
2. Modify existing scripts:
   ...
   - `overmind/scripts/project_mgmt/project_add_feature_e2e.sh` — insert `"8.2"` into `PHASE_IDS` and `PHASE_OPTIONAL` arrays; add step label, script name, and resume-alias handling
   - `overmind/scripts/project_mgmt/init_progress_scanner.sh` — recognize `prerequisite_gaps.md` as the Step `8.2` completion artifact
...
3. Update docs/templates:
   - `overmind/init_progress_definition_sequence_diagram.md` — add Step `8.2` node between `8.1` and `8.3`
   - `overmind/templates/init_progress_definition_TEMPLATE.yaml` — add `step_number: 8.2` block between `8.1` and `8.3`
...
4. Add and update tests:
   ...
   - `tests/ai_scripts/init_progress_scanner_tests.sh` — add Step `8.2` detection cases

Rollback: revert `project_add_feature_e2e.sh` and `init_progress_scanner.sh` to remove `8.2`; revert `check_implementation_plan_quality.sh` to remove the cross-check. Existing `prerequisite_gaps.md` artifacts are inert without the gate scripts.
