...
- [ ] 1.3 Add type `A` blueprint binding for `projects/<project-id>/project_stack_blueprint_<class>.md` as planned fallback evidence when no ready class repo path is available or when a ready repo path leaves rows/fields unresolved
...
- [ ] 2.1 Update Step `7` prompt construction to bind repository evidence and, for type `A`, planned blueprint fallback evidence when available; state that repo evidence wins per row/field
...
- [ ] 4.2 Add backend tests proving type `A` with a partially materialized ready backend repo uses repo evidence for materialized rows and planned blueprint evidence for unmaterialized rows in the same surface map
...
- [ ] 5.3 Add mixed target tests proving type `A` can use repo evidence for one active class, blueprint evidence for another active class, and repo-plus-blueprint fallback within a partially materialized class
