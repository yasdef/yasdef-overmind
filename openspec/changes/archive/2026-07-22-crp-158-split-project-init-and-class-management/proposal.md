...
- **BREAKING**: `overmind project create` captures project name, project-level `project_type_code`, and the class list only. Every selected class is written as `state: "deferred"`, `path: ""`, `policy: "A"`, meaning no existing repository is bound and feature work may proceed without repo evidence for that class. Selecting no class is valid.
...
- `overmind project reconcile` remains the sole writer of repository path and `state: "ready"` for existing repositories. Deferred policy `A` classes are intentionally repo-less and non-blocking, but reconcile can convert them to selected policy `B` or `C` and then ask for a path under the existing validation and retry rules; a supplied valid path records the selected policy with `state: "ready"`.
- `overmind run` allows deferred policy `A` classes, blocks deferred policy `B`/`C` classes with repo-binding guidance, and blocks ready-unreconciled classes with contract-reconciliation guidance.
...
- `project-update`: reconcile binds deferred existing-repo classes and is no longer the only project-update verb.
- `project-reconciliation-flow`: reconciliation commits use a neutral message that covers policy updates, repository bindings, and contract reconciliation state.
...
- `packages/asdlc-coordinator/src/orchestrator/run-project-reconciliation-flow.ts` — prompt deferred classes for policy, keep policy `A` non-blocking when selected, and bind selected policy `B`/`C` classes through the existing path prompt.
