...
import {
  ...
  validateTechnicalRequirementsContent
} from "../src/validate/technical-requirements.js";
  ...
  for (const heading of [
    ...
    assert.ok(
      problems(validArtifact().replace(heading, `${heading} renamed`)).some((p) =>
        p.includes(`missing section ${heading.slice(3)}`)
      ),
      heading
    );
  }
...
test("technical-requirements gate: all section 1 and 2 scalar keys are required", () => {
  for (const key of [
    ...
    assert.ok(
      problems(removeLine(validArtifact(), key)).some((p) => p.includes(`section 1 key ${key}`)),
      key
    );
  }
  for (const key of ["feature_summary", "included_behavior", "excluded_behavior"]) {
    assert.ok(
      problems(removeLine(validArtifact(), key)).some((p) => p.includes(`section 2 key ${key}`)),
      key
    );
  }
});
...
test("technical-requirements gate: repository coverage and every block field are required", () => {
  ...
  for (const key of [
    ...
    const broken = validArtifact().replace(
      new RegExp(`(### Repository: Backend[\\s\\S]*?)- ${key}:.*\\n`),
      `$1`
    );
    assert.ok(
      problems(broken).some((p) => p.includes(`repository Backend has unfilled key ${key}`)),
      key
    );
  }
});
  ...
  assert.ok(
    problems(
      validArtifact().replace(
        "- transport_layer: Service.call",
        "- current_state: Service and route exist"
      )
    ).some((p) => p.includes("conflated current_state"))
  );
  for (const key of ["transport_layer", "user_reachable_surface"]) {
    const broken = validArtifact().replace(
      new RegExp(`(### Requirement: REQ-1[\\s\\S]*?)- ${key}:.*\\n`),
      `$1`
    );
    assert.ok(
      problems(broken).some((p) => p.includes(`missing ${key} subfield`)),
      key
    );
  }
  assert.ok(
    problems(validArtifact().replace("gap_status: fully_implemented", "gap_status: unknown")).some(
      (p) => p.includes("invalid gap_status")
    )
  );
  assert.ok(
    problems(validArtifact().replace("repo_impact: backend", "repo_impact: infrastructure")).some(
      (p) => p.includes("invalid repo_impact")
    )
  );
});
...
test("technical-requirements gate: component allocation, kinds, and references are enforced", () => {
  ...
  assert.ok(
    problems(withoutFrontendComponent).some((p) =>
      p.includes("repo frontend has applicable touched surfaces")
    )
  );
  assert.ok(
    problems(validArtifact().replace("component_kind: service", "component_kind: handler")).some(
      (p) => p.includes("invalid component_kind")
    )
  );
  assert.ok(
    problems(validArtifact().replace("requirement_refs: REQ-1", "requirement_refs: REQ-99")).some(
      (p) => p.includes("unknown requirement id REQ-99")
    )
  );
});
...
- source_evidence: REQ-1, comp/thing-service`;
  ...
  assert.ok(
    problems(validArtifact().replace("- planning_signals: none", "- constraint_1: legacy")).some(
      (p) => p.includes("retired loose-entry")
    )
  );
  assert.ok(
    problems(validArtifact().replace("- planning_signals: none", "")).some((p) =>
      p.includes("section 6 must contain")
    )
  );
  assert.ok(
    problems(
      validArtifact().replace("- planning_signals: none", `- planning_signals: none\n${signal}`)
    ).some((p) => p.includes("cannot mix"))
  );
  assert.ok(
    problems(
      validArtifact().replace(
        "- planning_signals: none",
        signal.replace("cross_repo_contract_lock", "optional")
      )
    ).some((p) => p.includes("unsupported signal_type"))
  );
  assert.ok(
    problems(
      validArtifact().replace(
        "- planning_signals: none",
        signal.replace("owner_repo: backend", "owner_repo: infrastructure")
      )
    ).some((p) => p.includes("owner_repo"))
  );
  assert.ok(
    problems(
      validArtifact().replace(
        "- planning_signals: none",
        signal.replace("consumer_repos: frontend", "consumer_repos: infrastructure")
      )
    ).some((p) => p.includes("consumer_repos"))
  );
  assert.ok(
    problems(validArtifact().replace("- planning_signals: none", `${signal}\n\n${signal}`)).some(
      (p) => p.includes("duplicate planning signal id")
    )
  );
  assert.ok(
    problems(
      validArtifact().replace(
        "- planning_signals: none",
        signal.replace("comp/thing-service", "comp/missing")
      )
    ).some((p) => p.includes("unknown source_evidence token comp/missing"))
  );
});

test("technical-requirements gate: risks and placeholders are enforced", () => {
  assert.ok(
    problems(removeLine(validArtifact(), "risk_1")).some((p) =>
      p.includes("at least one explicit risk")
    )
  );
  assert.ok(
    problems(validArtifact().replace("- feature_id: F-1", "- feature_id: [UNFILLED]")).some((p) =>
      p.includes("[UNFILLED]")
    )
  );
});
