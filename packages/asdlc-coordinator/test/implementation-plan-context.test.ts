...
test("implementation-plan context emits runtime bindings, six read-only inputs, assets, and active classes", () => {
  ...
  try {
    ...
    for (const value of [
      ...
      "backend",
      "frontend",
      "assets/implementation_plan_TEMPLATE.md",
      "assets/implementation_plan_GOLDEN_EXAMPLE.md",
    ...
    for (const file of [
      ...
      assert.match(text, new RegExp(`read_only_input: .*${file}`));
...
test("implementation-plan context rejects unsupported classes and missing inputs", () => {
  ...
  for (const missing of [
    ...
    try {
      ...
      assert.equal(
        buildImplementationPlanContext("projects/p1/feature-a", root).exitCode,
        2,
        missing
      );
    } finally {
  ...
  try {
    assert.equal(buildImplementationPlanContext("projects/p1/missing", root).exitCode, 2);
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});
