...
import {
  ...
  validatePrerequisiteGapsContent
} from "../src/validate/prerequisite-gaps.js";
...
test("prerequisite-gaps gate: valid statuses and all literal coverage pass", () => {
  const all = artifact(
    presentBlock + scheduledBlock + siblingBlock,
    "Orders endpoint; Login route; Export command"
  );
  assert.deepEqual(validatePrerequisiteGapsContent(all, "WHEN POST /orders and `/login`", ""), []);
  assert.deepEqual(
    validatePrerequisiteGapsContent(
      sibling.replace("scheduled_in_feature sibling/8.3", "scheduled_in_feature   sibling/8.3"),
      "",
      ""
    ),
    []
  ...
  assert.deepEqual(
    validatePrerequisiteGapsContent(
      scheduled.replace("Operator login page", "Operator sync CLI command"),
      "",
      ""
    ),
    []
...
test("prerequisite-gaps gate: surface kind, status, identity, evidence, and slice rules are enforced", () => {
  ...
  for (const [input, from, to, expected] of mutations) {
    assert.match(
      validatePrerequisiteGapsContent(input.replace(from, to), "", "").join("\n"),
      expected
    );
  }
  assert.match(
    validatePrerequisiteGapsContent(
      present.replace("surface_identity: none", "surface_identity: Orders page"),
      "",
      ""
    ).join("\n"),
  ...
  assert.match(
    validatePrerequisiteGapsContent(
      present.replace("status: present_in_repo", "status: scheduled_in_slices"),
      "",
      ""
    ).join("\n"),
  ...
  assert.match(
    validatePrerequisiteGapsContent(
      scheduled.replace(/- evidence:.*/, "- evidence: [UNFILLED]"),
      "",
      ""
    ).join("\n"),
  ...
  assert.match(
    validatePrerequisiteGapsContent(
      scheduled.replace("slice_ref: slice-1", "slice_ref: none"),
      "",
      ""
    ).join("\n"),
    /missing slice_ref/
  ...
  assert.match(
    validatePrerequisiteGapsContent(scheduled.replace("- slice_ref: slice-1", ""), "", "").join(
      "\n"
    ),
    /missing slice_ref/
  );
  assert.match(
    validatePrerequisiteGapsContent(
      sibling.replace(/- evidence:.*/, "- evidence: [UNFILLED]"),
      "",
      ""
    ).join("\n"),
  ...
  assert.match(
    validatePrerequisiteGapsContent(
      sibling.replace("slice_ref: none", "slice_ref: slice-2"),
      "",
      ""
    ).join("\n"),
...
`;
  assert.deepEqual(
    validatePrerequisiteGapsContent(
      artifact(presentBlock, "Orders endpoint", sharedCoverage),
      "WHEN POST /orders",
      ""
    ),
    []
...
test("prerequisite-gaps gate: requirement coverage blocks carry required fields and do not contain catalog blocks", () => {
  assert.match(
    validatePrerequisiteGapsContent(presentBlock, "", "").join("\n"),
  ...
  assert.match(
    validatePrerequisiteGapsContent(
      present.replace("- requirement_summary: Exercise the covered behavior.\n", ""),
      "",
      ""
    ).join("\n"),
  ...
  assert.match(
    validatePrerequisiteGapsContent(
      present.replace("- prerequisites: Orders endpoint", "- prerequisites:"),
      "",
      ""
    ).join("\n"),
...
`;
  assert.match(
    validatePrerequisiteGapsContent(coverageEntry, "", "").join("\n"),
...
test("prerequisite-gaps gate: literal may be covered by catalog entry or technical surface", () => {
  ...
  assert.deepEqual(
    validatePrerequisiteGapsContent(
      present,
      "WHEN GET /lookup",
      "- user_reachable_surface: GET /lookup"
    ),
    []
  );
  assert.match(
    validatePrerequisiteGapsContent(present, "WHEN GET /missing", "").join("\n"),
    /GET \/missing/
  );
});
...
test("prerequisite-gaps gate: literal haystack includes catalog evidence and slice_ref only", () => {
  ...
  assert.match(
    validatePrerequisiteGapsContent(outOfCatalog, "WHEN POST /admin/reindex", "").join("\n"),
    /POST \/admin\/reindex/
  );
  ...
  assert.deepEqual(
    validatePrerequisiteGapsContent(repeatedEvidence, "WHEN POST /admin/reindex", ""),
    []
  );
});
...
test("prerequisite-gaps gate: slice format only applies to scheduled_in_slices", () => {
  ...
  assert.match(
    validatePrerequisiteGapsContent(scheduled.replace("slice-1", "bad ref"), "", "").join("\n"),
    /required format/
  );
  assert.deepEqual(
    validatePrerequisiteGapsContent(
      present.replace("slice_ref: none", "slice_ref: bad ref"),
      "",
      ""
    ),
    []
