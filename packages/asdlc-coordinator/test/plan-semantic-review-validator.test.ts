...
import {
  ...
  validatePlanSemanticReviewContent
} from "../src/validate/plan-semantic-review.js";

function meta(status = "complete"): string {
  return `## 1. Document Meta
- feature_id: F-1
...
function finding(overrides: Record<string, string> = {}): string {
  const fields = {
    ...
    state: "applied",
    target_steps: "Step 1.1",
    related_requirements: "REQ-1",
    related_evidence: "gap/TECH_REQ-1",
    summary: "Summary",
    rationale: "Rationale",
    recommendation: "Recommendation",
    user_selection: "selected",
    plan_patch_summary: "Patched",
...
function validNoFindings(): string {
  return `${meta()}\n- no_findings: true\n`;
}
function messages(content: string): string {
  return validatePlanSemanticReviewContent(content).join("\n");
}
...
test("plan-semantic-review gate requires all sections", () => {
  for (const heading of ["## 1. Document Meta", "## 2. Review Guidance", "## 3. Findings Ledger"]) {
    assert.match(
      messages(validNoFindings().replace(heading, `## Missing ${heading}`)),
      /missing section:/
    );
  }
});
...
test("plan-semantic-review gate enforces meta and ledger consistency", () => {
  assert.match(
    messages(validNoFindings().replace("- feature_id: F-1", "- feature_id: [UNFILLED]")),
    /missing or unfilled meta key: feature_id/
  );
  assert.match(
    messages(validNoFindings().replace("review_status: complete", "review_status: done")),
    /review_status must be/
  );
  assert.match(messages(`${meta()}\n`), /no_findings: true/);
  assert.match(
    messages(`${meta("in_progress")}\n- no_findings: true\n`),
    /review_status must be complete/
  );
  assert.match(
    messages(`${meta()}\n- no_findings: true\n${finding()}`),
    /no_findings must not be true/
  );
});
...
test("plan-semantic-review gate enforces every finding field and enum", () => {
  assert.match(
    messages(`${meta()}\n${finding({ summary: "" })}`),
    /missing or unfilled key: summary/
  );
  assert.match(messages(`${meta()}\n${finding({ severity: "Critical" })}`), /invalid severity/);
  assert.match(
    messages(`${meta()}\n${finding({ finding_type: "style" })}`),
  ...
  for (const findingType of [
    ...
    assert.match(
      messages(
        `${meta()}\n${finding({ finding_type: findingType, resolution_notes: "<decision and final outcome>" })}`
      ),
      /terminal state with empty resolution_notes/
    );
  }
  assert.match(
    messages(
      `${meta()}\n${finding({ finding_type: "delivered_surface_consumption_unclear", related_requirements: "FR-1" })}`
    ),
...
test("plan-semantic-review gate rejects complete non-terminal findings and placeholders", () => {
  assert.match(
    messages(`${meta()}\n${finding({ state: "added" })}`),
    /non-terminal findings remain/
  );
  assert.match(
    messages(validNoFindings().replace("Feature", "[UNFILLED]")),
    /artifact still contains \[UNFILLED\]/
  );
});
...
test("plan-semantic-review gate preserves runtime and empty-content exit codes", () => {
  ...
  try {
    ...
    writeFileSync(target, " \n\t");
    assert.equal(validatePlanSemanticReview("projects/p1/feature-a", root).exitCode, 1);
    writeFileSync(target, validNoFindings());
    assert.equal(validatePlanSemanticReview("projects/p1/feature-a", root).exitCode, 0);
  } finally {
