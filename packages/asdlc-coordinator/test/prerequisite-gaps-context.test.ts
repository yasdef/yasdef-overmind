...
test("prerequisite-gaps context emits paths, classes, assets, gate, and sibling manifest", () => {
  ...
  try {
    ...
    for (const value of [
      ...
      "node .overmind/overmind.js gate prerequisite-gaps projects/p1/feature-a",
      "assets/prerequisite_gaps_TEMPLATE.md",
      "assets/prerequisite_gaps_GOLDEN_EXAMPLE.md",
      "projects/p1/sibling/implementation_plan.md",
      "- backend",
      "- frontend"
    ])
...
test("prerequisite-gaps context omits absent siblings, skips infrastructure, and rejects unsupported classes", () => {
  ...
  try {
    ...
    assert.doesNotMatch(ok.text ?? "", /- infrastructure/);
    writeFileSync(
      path.join(root, "projects", "p1", "init_progress_definition.yaml"),
      "meta_info:\n  project_classes: [backend, bakend]\nsteps: []\n"
    );
    assert.match(
      buildPrerequisiteGapsContext("projects/p1/feature-a", root).errorMessage ?? "",
      /bakend/
    );
  } finally {
    rmSync(root, { recursive: true, force: true });
...
test("prerequisite-gaps context rejects missing inputs and invalid feature layout", () => {
  ...
  try {
    ...
    assert.match(
      buildPrerequisiteGapsContext("feature", root).errorMessage ?? "",
      /projects\/<project-id>/
    );
  } finally {
    rmSync(root, { recursive: true, force: true });
