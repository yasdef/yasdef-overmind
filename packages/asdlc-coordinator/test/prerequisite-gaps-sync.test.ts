...
function definition(root: string, entries: Array<[string, string]>): string {
  ...
  mkdirSync(path.join(project, "f1"), { recursive: true });
...
test("prerequisite-gaps sync syncs supported repos and filters infrastructure before path validation", () => {
  ...
  try {
    const backend = repo(root, "backend");
    const frontend = repo(root, "frontend");
    const def = definition(root, [
      ["backend", backend],
      ["frontend", frontend],
      ["infrastructure", path.join(root, "missing")]
    ]);
    assert.deepEqual(
      collectReadyRepoPaths(def, ["BACKEND", "frontend"]).map((item) => item.class),
      ["backend", "frontend"]
    );
...
test("prerequisite-gaps sync reports blocked supported repos and missing definition", () => {
  ...
  try {
    const backend = repo(root, "backend");
    definition(root, [["backend", backend]]);
    writeFileSync(path.join(backend, "dirty"), "x");
    const blocked = syncPrerequisiteGapsStep("projects/p1/f1", root);
    assert.equal(blocked.exitCode, 2);
    assert.match(blocked.blockedMessages?.[0] ?? "", /BLOCKED:/);
    rmSync(path.join(root, "projects", "p1", "init_progress_definition.yaml"));
    assert.match(
      syncPrerequisiteGapsStep("projects/p1/f1", root).errorMessage ?? "",
