...
test("implementation-plan gate: valid multi-repo plan and canonical surface matches pass", () => {
  ...
  assert.equal(implementationPlanSurfaceMatches("order query CLI command", "order query admin tool command"), true);
...
test("implementation-plan gate: structural failures are reported", () => {
  const cases: Array<[string, string]> = [
    ...
    [validPlan().replace("#### Evidence: gap/TECH_REQ-1, comp/backend-order-service, slice/slice-1", "#### Evidence: gap/TECH_REQ-1\n#### Evidence: comp/backend-order-service, slice/slice-1"), "more than once"],
  ...
  for (const [content, expected] of cases) assert.ok(validateImplementationPlanContent(content, catalogs).some((p) => p.includes(expected)), expected);
});
...
test("implementation-plan gate: empty Repo and Evidence diagnostics preserve legacy message parity", () => {
  const emptyEvidence = validateImplementationPlanContent(
    validPlan().replace("#### Evidence: gap/TECH_REQ-1, comp/backend-order-service, slice/slice-1", "#### Evidence:"),
    catalogs
  );
  assert.deepEqual(emptyEvidence.filter((problem) => problem.includes("#### Evidence")), ["step 1.1 is missing #### Evidence"]);

  const emptyRepo = validateImplementationPlanContent(validPlan().replace("#### Repo: backend", "#### Repo:"), catalogs);
  assert.deepEqual(emptyRepo.filter((problem) => problem.includes("step 1.1") && /repo/i.test(problem)), [
...
test("implementation-plan gate: dependency and evidence token failures are reported", () => {
  const cases: Array<[string, string]> = [
    ...
    [validPlan().replace("#### Depends on: 1.1", "#### Depends on: ../1.1"), "invalid cross-feature"],
    [validPlan().replace("#### Depends on: 1.1", "#### Depends on: 1.1, 1.1"), "repeats dependency"],
    ...
    [validPlan().replace("gap/TECH_REQ-1, comp/backend-order-service, slice/slice-1", "bad-token"), "invalid evidence token"],
    [validPlan().replace("gap/TECH_REQ-1, comp/backend-order-service, slice/slice-1", "gap/TECH_REQ-1,"), "empty evidence token"],
    [validPlan().replace("gap/TECH_REQ-1, comp/backend-order-service, slice/slice-1", "gap/TECH_REQ-1, gap/TECH_REQ-1"), "repeats evidence token"]
  ];
  for (const [content, expected] of cases) assert.ok(validateImplementationPlanContent(content, catalogs).some((p) => p.includes(expected)), expected);
  assert.deepEqual(validateImplementationPlanContent(validPlan().replace("#### Depends on: 1.1", "#### Depends on: sibling-feature/1.1"), catalogs), []);
});
...
test("implementation-plan gate: whole-plan coverage and preserved surfaces are enforced", () => {
  const cases: Array<[string, ImplementationPlanCatalogs, string]> = [
    ...
    [validPlan().replace("comp/frontend-order-page", "gap/TECH_REQ-NFR-1"), catalogs, "unresolved component"],
  ...
  for (const [content, input, expected] of cases) assert.ok(validateImplementationPlanContent(content, input).some((p) => p.includes(expected)), expected);
  const coordination = validPlan().replace("#### Preserved Surface: Operator sign-in screen", "#### Preserved Surface: Operator sign-in screen\n#### Coordination: true");
  assert.ok(validateImplementationPlanContent(coordination, catalogs).some((p) => p.includes("no non-coordination")));
  const beside = coordination.replace("#### Preserved Surface: none", "#### Preserved Surface: Admin login page");
...
test("implementation-plan gate: catalog extractors preserve section and heading scope", () => {
  assert.deepEqual(extractImplementationPlanRequirementRefs("### Requirement 1 text\nbody REQ-9\n### NFR 2 [REQ-3]\n"), ["REQ-1", "NFR-2", "REQ-3"]);
...
`;
  assert.deepEqual(extractTechnicalEvidenceCatalog(technical), {
    reqAll: ["gap/TECH_REQ-1", "gap/TECH_REQ-NFR-1"], reqUnresolved: ["gap/TECH_REQ-1"],
    compAll: ["comp/backend-order-service", "comp/existing-page"], compUnresolved: ["comp/backend-order-service"], repoUnresolved: ["backend"]
  });
...
function fixture(root: string): { project: string; feature: string } {
  ...
  writeFileSync(path.join(feature, "technical_requirements.md"), `## 4. Requirement Coverage and Gaps
### Requirement: REQ-1
- gap_status: pending
- gap_to_close: implement
### Requirement: NFR-1
...
- gap_to_close: implement
`);
  writeFileSync(path.join(feature, "prerequisite_gaps.md"), `#### Prerequisite: A
- status: scheduled_in_slices
- slice_ref: slice-1
- surface_kind: required_missing_user_reachable_surface
- surface_identity: Admin login page
...
test("implementation-plan gate: runtime exit codes and required inputs", () => {
  ...
  for (const missing of ["requirements_ears.md", "technical_requirements.md", "prerequisite_gaps.md", "definition"]) {
    root = mkdtempSync(path.join(tmpdir(), "plan-gate-missing-"));
    try {
      const { project, feature } = fixture(root);
      unlinkSync(missing === "definition" ? path.join(project, "init_progress_definition.yaml") : path.join(feature, missing));
