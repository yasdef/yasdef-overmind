...
function fixture(root: string, classes = "[backend, frontend]"): { project: string; feature: string } {
  ...
  for (const klass of ["backend", "frontend"]) writeFileSync(path.join(feature, `project_surface_struct_resp_map_${klass}.md`), "surface\n");
...
test("technical-requirements context: missing required inputs exit 2", () => {
  for (const missing of ["requirements_ears.md", "common_contract_definition.md", "project_surface_struct_resp_map_frontend.md"]) {
    const root = mkdtempSync(path.join(tmpdir(), "technical-context-missing-"));
    try {
      const { project, feature } = fixture(root);
      const file = missing === "common_contract_definition.md" ? path.join(project, missing) : path.join(feature, missing);
      unlinkSync(file);
      const result = buildTechnicalRequirementsContext("projects/p1/feature-a", root);
