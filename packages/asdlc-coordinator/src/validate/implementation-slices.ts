...
const META_KEYS = [
  "feature_id", "feature_title", "project_type_code", "source_requirements_ears",
  "source_technical_requirements", "source_feature_contract_delta", "source_surface_map_artifacts",
  "analyzed_repo_classes", "ordering_scope", "traceability_scope", "last_updated", "confidence_level"
] as const;
const SLICE_FIELDS = ["repo", "status", "objective", "first_increment", "prerequisites", "preserved_operator_surface", "evidence"] as const;
const HANDOFF_KEYS = ["ordering_intent", "unresolved_ordering_questions", "unresolved_traceability_questions"] as const;
...
export function validateImplementationSlices(inputPath: string, cwd = process.cwd()): GateResult {
  ...
  if (!isFile(targetPath)) return gateError(`Target implementation slices artifact not found: ${displayPath(targetPath, workspaceRoot)}`);
  const content = readFileSync(targetPath, "utf8");
  if (!/\S/.test(content)) return gateFailure([`target implementation slices artifact is empty: ${displayPath(targetPath, workspaceRoot)}`]);
  ...
  try {
    const activeClasses = new Set(parseImplementationSlicesProjectClasses(definitionPath).filter((item) => SURFACE_CLASSES.has(item)));
    ...
    return problems.length === 0
      ? { exitCode: 0, passMessage: "quality gate passed: implementation slices structure is complete", problems: [] }
...
export function validateImplementationSlicesContent(
  ...
  for (const rawLine of content.split(/\r?\n/)) {
    ...
    if (section === "3" && sliceHeading) {
      currentSlice = { number: slices.length + 1, heading: normalize(rawLine), fields: new Map(), bullets: [] };
    ...
    if (section === "3" && currentSlice && bullet) {
      ...
      if (text === "Plan and discuss the slice" || text === "Review slice readiness") {
        fail(`slice ${currentSlice.number} contains forbidden lifecycle boilerplate bullet: ${text}`);
      }
      continue;
    }
  ...
  for (const [number, label] of [["1", "## 1. Document Meta"], ["2", "## 2. Slice Planning Guardrails"], ["3", "## 3. Slice Candidates"], ["4", "## 4. Handoff To Ordered Plan"]] as const) {
    if (!seenSections.has(number)) fail(`missing section: ${label}`);
  }
  for (const key of META_KEYS) if (unfilled(meta.get(key))) fail(`missing or unfilled meta key: ${key}`);
  if ((meta.get("ordering_scope") ?? "").toLowerCase() !== "local_prerequisites_only") fail("ordering_scope must be local_prerequisites_only");
  if ((meta.get("traceability_scope") ?? "").toLowerCase() !== "slice_level_only") fail("traceability_scope must be slice_level_only");

  let plannedCount = 0;
  const coveredSurfaces: string[] = [];
  for (const slice of slices) {
    for (const key of SLICE_FIELDS) if (unfilled(slice.fields.get(key))) fail(`slice ${slice.number} missing or unfilled key: ${key}`);
    const repo = (slice.fields.get("repo") ?? "").toLowerCase();
    if (!activeClasses.has(repo)) fail(`slice ${slice.number} uses repo outside active project classes: ${repo}`);
    ...
    if (status !== "existing" && status !== "planned") fail(`slice ${slice.number} has invalid status: ${slice.fields.get("status") ?? ""}`);
    ...
    for (const rawToken of evidenceTokens) {
      ...
      } else if (/^gap\/TECH_REQ-([0-9]+|NFR-[0-9]+)$/.test(token) || /^comp\/[a-z0-9]+(?:-[a-z0-9]+)*$/.test(token)) {
        validEvidence += 1;
      } else {
        fail(`slice ${slice.number} has invalid evidence token: ${token}`);
      }
    }
    if (validEvidence === 0) fail(`slice ${slice.number} must include at least one valid evidence token`);
    if ((slice.fields.get("kind") ?? "").toLowerCase() === "coordination" && unfilled(slice.fields.get("signal_ref"))) {
      fail(`slice ${slice.number} has kind: coordination but signal_ref is missing or empty`);
    }
    if (slice.bullets.length < 2) fail(`slice ${slice.number} must include at least 2 concrete checklist bullets`);

    const preservedSurface = (slice.fields.get("preserved_operator_surface") ?? "").trim();
    if (preservedSurface.toLowerCase() !== "none") {
      if (!hasSurfaceTerms(preservedSurface)) fail(`slice ${slice.number} preserved_operator_surface is not operator-facing: ${preservedSurface}`);
      const coverageText = [slice.heading, slice.fields.get("objective"), slice.fields.get("first_increment"), ...slice.bullets].join(" ").toLowerCase();
      if (looksSupportingOnly(coverageText)) fail(`slice ${slice.number} marks preserved_operator_surface but describes supporting-only scaffolding work`);
  ...
  for (const requiredSurface of requiredSurfaces) {
    if (!coveredSurfaces.some((candidate) => surfaceMatches(requiredSurface, candidate))) {
      fail(`required missing operator-facing surface is not preserved by any slice: ${requiredSurface}`);
    }
  }
  for (const key of HANDOFF_KEYS) if (unfilled(handoff.get(key))) fail(`missing or unfilled handoff key: ${key}`);
  return problems;
}
...
export function extractRequiredMissingSurfaces(content: string): string[] {
  ...
  let surfaceIdentity = "";
  const flush = (): void => {
    if (inPrerequisite && (status === "scheduled_in_slices" || status === "unmet") && surfaceKind === "required_missing_user_reachable_surface" && !unfilledOrNone(surfaceIdentity)) {
...
export function hasSurfaceTerms(value: string): boolean {
  return /(?:login|shell|route|lookup|page|workspace|form|command|job|endpoint|tool|link)/.test(canonicalSurface(value));
}

export function looksSupportingOnly(value: string): boolean {
  const lower = value.toLowerCase();
  const hasSupport = /(?:auth|token|api|contract|schema|state|coordination|middleware|service|repository|adapter|dto|mapper|payload)/.test(lower);
  const hasSurface = /(?:login|sign[ -]?in|route|page|screen|shell|workspace|entry|lookup|search|dashboard|portal|console|form|command|cli|job|endpoint|tool|http|deep link|deeplink)/.test(lower);
...
export function surfaceMatches(requiredValue: string, candidateValue: string): boolean {
  ...
  if (requiredSpecific > 0) return requiredContent > 0 ? sharedSpecific > 0 && sharedContent > 0 : sharedSpecific > 0;
