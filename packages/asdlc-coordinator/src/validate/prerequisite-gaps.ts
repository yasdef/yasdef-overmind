...
export function validatePrerequisiteGaps(inputPath: string, cwd = process.cwd()): GateResult {
  ...
  if (!isFile(targetPath))
    return gateError(
      `Target prerequisite gaps artifact not found: ${displayPath(targetPath, workspaceRoot)}`
    );
  const content = readFileSync(targetPath, "utf8");
  if (!/\S/.test(content))
    return gateFailure([
      `target prerequisite gaps artifact is empty: ${displayPath(targetPath, workspaceRoot)}`
    ]);
  if (!isFile(requirementsPath))
    return gateError(
      `Required sibling artifact not found for quality check: ${displayPath(requirementsPath, workspaceRoot)}`
    );
  if (!isFile(technicalPath))
    return gateError(
      `Required sibling artifact not found for quality check: ${displayPath(technicalPath, workspaceRoot)}`
    );
...
export function validatePrerequisiteGapsContent(
  content: string,
  requirements: string,
  technical: string
): string[] {
  const problems: string[] = [];
  if (!/^## 2\. Prerequisite Catalog\s*$/m.test(content))
    problems.push("missing section: ## 2. Prerequisite Catalog");
  if (!/^## 3\. Requirement Coverage\s*$/m.test(content))
    problems.push("missing section: ## 3. Requirement Coverage");
  ...
  for (const block of blocks) {
    if (catalogNames.has(block.name))
      problems.push(`catalog prerequisite is declared more than once: "${block.name}"`);
    catalogNames.add(block.name);
  }

  const referencedNames = new Set<string>();
  for (const requirement of parseRequirementCoverage(coverage)) {
    if (unfilled(requirement.summary))
      problems.push(`requirement ${requirement.name} is missing requirement_summary`);
    ...
    for (const reference of requirement.prerequisites.split(/\s*;\s*/).filter(Boolean)) {
      ...
      if (!catalogNames.has(reference))
        problems.push(
          `prerequisite reference does not resolve to any catalog entry: "${reference}"`
        );
    }
  }
  for (const name of catalogNames) {
    if (!referencedNames.has(name))
      problems.push(`catalog prerequisite is referenced by no requirement: "${name}"`);
  }
  ...
  for (const literal of extractEarsLiterals(requirements)) {
    if (![...entries, ...surfaces].some((value) => value.includes(literal))) {
      problems.push(
        `literal "${literal}" from requirements_ears.md is absent from both prerequisite_gaps.md entries and user_reachable_surface in technical_requirements.md`
      );
    }
  }
  return problems;
...
function rejectCatalogContentOutsideCatalog(content: string, problems: string[]): void {
  ...
  for (const line of content.split(/\r?\n/)) {
    ...
    const catalogField = line.match(
      /^\s*-\s*(status|surface_kind|surface_identity|evidence|slice_ref):/
    );
...
function parsePrerequisites(content: string): PrerequisiteBlock[] {
  ...
  const flush = (): void => {
    if (current && current.name !== "") blocks.push(current);
    current = undefined;
  };
  for (const line of content.split(/\r?\n/)) {
    ...
    if (!current) continue;
    const kv = line.match(
      /^\s*-\s*(status|surface_kind|surface_identity|evidence|slice_ref):\s*(.*)$/
    );
    if (!kv) continue;
...
function validateBlock(block: PrerequisiteBlock, problems: string[]): void {
  ...
  if (unfilled(surfaceKind)) problems.push(`${prefix} is missing surface_kind`);
    ...
    ![
      "required_missing_user_reachable_surface",
      "present_user_reachable_surface",
      "transport_or_internal_execution_gap"
  ...
  if (unfilled(status)) problems.push(`${prefix} is missing status`);
    ...
    problems.push(
      `catalog has unmet prerequisite: "${name}" — resolve by adding a slice to implementation_slices.md`
    );
    ...
    if (unfilled(sliceRef) || sliceRef === "none")
      problems.push(`${prefix} (scheduled_in_slices) is missing slice_ref`);
  } else if (scheduledInFeature(status)) {
    if (unfilled(evidence)) problems.push(`${prefix} (${status}) is missing evidence`);
    if (!unfilled(sliceRef) && sliceRef !== "none")
      problems.push(`${prefix} (${status}) must use slice_ref: none`);
  ...
  if (surfaceKind === "required_missing_user_reachable_surface") {
    if (status !== "unmet" && status !== "scheduled_in_slices" && !scheduledInFeature(status)) {
      problems.push(
        `${prefix} uses required_missing_user_reachable_surface but status is not unmet/scheduled_in_slices/scheduled_in_feature`
      );
    }
    if (unfilled(surfaceIdentity) || surfaceIdentity.toLowerCase() === "none")
      problems.push(
        `${prefix} (required_missing_user_reachable_surface) is missing surface_identity`
      );
    else if (!looksLikeSurfaceIdentity(surfaceIdentity))
      problems.push(`${prefix} has non-operator-facing surface_identity: "${surfaceIdentity}"`);
  } else if (surfaceKind === "present_user_reachable_surface") {
    if (status !== "present_in_repo")
      problems.push(
        `${prefix} uses present_user_reachable_surface but status is not present_in_repo`
      );
    if (!unfilled(surfaceIdentity) && surfaceIdentity.toLowerCase() !== "none")
      problems.push(`${prefix} (present_user_reachable_surface) must use surface_identity: none`);
  } else if (surfaceKind === "transport_or_internal_execution_gap") {
    problems.push(
      `${prefix} is classified as transport_or_internal_execution_gap; keep transport/internal gaps out of prerequisite entries`
    );
  }
  ...
  if (
    ...
    !unfilled(sliceRef) &&
    sliceRef !== "none" &&
    !/^[A-Za-z0-9][A-Za-z0-9_.-]*$/.test(sliceRef)
  ) {
    problems.push(
      `slice_ref "${sliceRef}" in catalog prerequisite "${name}" does not match required format [A-Za-z0-9][A-Za-z0-9_.-]*`
    );
  }
}

function looksLikeSurfaceIdentity(value: string): boolean {
  return /(route|page|screen|shell|login|sign-in|signin|workspace|entry|portal|console|ui|view|lookup|search|dashboard|form|command|cli|job|endpoint|tool|http|post |get |put |patch |delete |deep link|deeplink)/.test(
