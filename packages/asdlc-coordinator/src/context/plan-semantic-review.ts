...
export function buildPlanSemanticReviewContext(inputPath: string, cwd = process.cwd()): ContextResult {
  ...
  for (const required of [definitionPath, requirementsPath, technicalPath, prerequisitePath, planPath]) {
    if (!isFile(required)) return contextError(`Required file not found: ${displayPath(required, workspaceRoot)}`);
  }
  ...
  try {
    ...
    if (unsupported) {
      return contextError(`Unsupported project class '${unsupported}' in ${displayPath(definitionPath, workspaceRoot)}; expected backend, frontend, mobile, or infrastructure`);
    }
    ...
    if (missingMaps.length > 0) {
      return contextError(`Required surface-map artifacts not found for active repo classes: ${missingMaps.join(" ")}`);
    }

    const featurePath = displayPath(featureDir, workspaceRoot);
    const readOnlyInputs = [definitionPath, requirementsPath, technicalPath, prerequisitePath, ...surfaceMaps.map(({ file }) => file)];
    const lines = [
      "# plan-semantic-review context", "", "## Runtime Paths",
