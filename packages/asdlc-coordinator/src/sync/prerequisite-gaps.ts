...
const SUPPORTED_CLASSES = ["backend", "frontend", "mobile"];

export function syncPrerequisiteGapsStep(inputPath: string, cwd = process.cwd()): SyncStepResult {
  const resolved = resolveFeatureWithinWorkspace(inputPath, cwd);
  if (!resolved.ok) return { exitCode: 2, errorMessage: resolved.message };
  ...
  try {
    const readyRepos = collectReadyRepoPaths(definitionPath, SUPPORTED_CLASSES);
    const blocked: string[] = [];
    for (const repo of readyRepos) {
      const result = syncRepoToDefaultBranch(repo.path);
      if (!result.ok) blocked.push(result.blockedMessage);
    }
