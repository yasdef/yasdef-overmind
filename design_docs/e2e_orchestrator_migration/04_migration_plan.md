...
1. `overmind run`, `overmind status`, `overmind scaffold feature`, and `overmind project reconcile` are the only entrypoints for the feature and project flows; no `.sh` remains under `overmind/scripts/` for scanner, scaffold, e2e, attach, or reconciliation.
