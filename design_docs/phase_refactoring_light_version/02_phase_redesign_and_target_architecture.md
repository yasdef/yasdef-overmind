...
EARS stays a distinct formalization step (the ID currency is load-bearing — worker commits reference REQ ids). Change: the drift audit (old 5.1) becomes a mandatory embedded verification pass — fresh context, dual-source. It treats `feature_br_summary.md` as the authority for clarified business decisions and checks EARS against it, and it independently uses raw `user_br_input.md` as a drift backstop: every statement in the summary or EARS that narrows a raw obligation (an added condition, state, actor, or qualifier that permits behavior the raw story forbids, such as scoping a status-independent duplicate-account prohibition to an `ACTIVE` account) is a mandatory finding. Summary authority controls the recommended resolution, not whether the discrepancy is surfaced; the pass never silently overrides a clarified summary decision with raw wording. It writes findings + dispositions into the EARS document's own `## Verification` section; a raw-narrowing finding cites all three locations — the raw location, the corresponding summary location, and the affected EARS requirement — while a finding not motivated by raw drift records `none` for the raw reference. Findings live with what they qualify; the audit cannot be skipped.
