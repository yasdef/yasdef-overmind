...
BEGIN {
  ...
  expected_layers["3.1 API Layer"] = 1
  expected_layers["3.2 Application / Service Layer"] = 1
  expected_layers["3.3 Domain Layer"] = 1
  expected_layers["3.4 Persistence / Data Layer"] = 1
  expected_layers["3.5 Integration Layer"] = 1
  expected_layers["3.6 Runtime / Ops Layer"] = 1
  expected_layers["3.7 Test Layer"] = 1

  expected_surfaces["4.1 API Surface"] = 1
  expected_surfaces["4.2 Application / Service Surface"] = 1
  expected_surfaces["4.3 Domain Surface"] = 1
  expected_surfaces["4.4 Persistence / Data Surface"] = 1
  expected_surfaces["4.5 Integration Surface"] = 1
  expected_surfaces["4.6 Runtime / Ops Surface"] = 1
  expected_surfaces["4.7 Test Surface"] = 1
  expected_surfaces["4.8 Unexpected Backend Surface"] = 1
}
{
  line = trim($0)
  ...
  if (line == "# Project Surface Structure + Responsibility Map (Backend)") {
    saw_title = 1
  }
  ...
  if (line == "## 4. Backend Surfaces Touched With Current Feature") {
    saw_section_4 = 1
    in_meta = 0
    in_scope = 0
    current_layer = ""
...
END {
  if (!saw_title) fail_quality("unexpected title for backend surface map")
  ...
  if (!saw_section_4) fail_quality("missing section ## 4. Backend Surfaces Touched With Current Feature")
  ...
  if (meta_value["project_classes"] !~ /backend/) {
    fail_quality("project_classes must include backend")
  }
  ...
  if (touched_surface_count < 1) {
    fail_quality("at least one backend surface should be marked applicable")
  }

  if (has_errors) {
    exit 1
  }
  print "quality gate passed: backend repo surface map is complete enough"
}
' "$target_path"
  status=$?
  set -e
...
main() {
  ...
  if [[ ! -f "$surface_path" ]]; then
    helper_fail "Target backend surface map artifact not found: $surface_path"
  fi

  if ! grep -q '[^[:space:]]' "$surface_path"; then
    echo "quality gate failed: target backend surface map artifact is empty: $surface_path"
