...
TECHNICAL_REQUIREMENTS_TEMPLATE_FILE=".templates/technical_requirements_TEMPLATE.md"
TECHNICAL_REQUIREMENTS_GOLDEN_EXAMPLE_FILE=".golden_examples/technical_requirements_GOLDEN_EXAMPLE.md"
MODELS_FILE=".setup/models.md"
RULE_FILE=".rules/technical_requirements_rule.md"
QUALITY_GATE_HELPER=".helper/check_feature_technical_requirements_quality.sh"
MODEL_PHASE="feature_technical_requirements"
...
collect_applicable_surface_maps() {
  ...
  if [[ ${#APPLICABLE_REPO_CLASSES[@]} -eq 0 ]]; then
    die "No supported repo classes found for feature technical requirements in $PROJECT_DEFINITION_FILE."
  fi
}
...
ensure_required_files() {
  ...
  local required_paths=(
    ...
    "$MODELS_FILE"
    "$RULE_FILE"
    "$TECHNICAL_REQUIREMENTS_TEMPLATE_FILE"
    "$TECHNICAL_REQUIREMENTS_GOLDEN_EXAMPLE_FILE"
    "$QUALITY_GATE_HELPER"
  )
  local relative_path=""
...
failure_line() {
  printf '%s' "feature technical requirements gate cannot pass with current requirements/common-contract/surface-map inputs. Please provide instructions what to do, or adjust inputs and rerun this phase"
}

success_line() {
  printf '%s' "Feature technical requirements phase is finished. Nothing else to do now; press Ctrl-C so orchestrator can start the next phase"
}
...
build_prompt() {
  local runtime_root="$1"
  local quality_command="$QUALITY_GATE_HELPER $TECHNICAL_REQUIREMENTS_FILE"
  local failure_msg=""
  local success_msg=""
  ...
  cat <<EOF
Create one shared feature-scoped technical requirements artifact for this feature.

Hard constraints:
- Read and follow $RULE_FILE fully before editing.
- Treat $RULE_FILE as authoritative for this phase.
- Use $TECHNICAL_REQUIREMENTS_TEMPLATE_FILE as the structure contract for $TECHNICAL_REQUIREMENTS_FILE.
- Use $TECHNICAL_REQUIREMENTS_GOLDEN_EXAMPLE_FILE as the style contract for $TECHNICAL_REQUIREMENTS_FILE.
...
- After drafting the full artifact, review it once again and fix coverage/consistency mistakes before finishing.
...
$surface_lines
- Target artifact: $TECHNICAL_REQUIREMENTS_FILE
- Rule file: $RULE_FILE
- Template file: $TECHNICAL_REQUIREMENTS_TEMPLATE_FILE
- Golden example file: $TECHNICAL_REQUIREMENTS_GOLDEN_EXAMPLE_FILE
...
main() {
  ...
  output_path="$runtime_root/$TECHNICAL_REQUIREMENTS_FILE"
  ...
  if [[ ! -f "$output_path" ]]; then
    die "Model run did not produce required file: $TECHNICAL_REQUIREMENTS_FILE"
  fi
